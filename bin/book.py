...
class Book(search.Search):
    def type_name(self,fn_class='seleniumFirstName',
                 ln_class='seleniumLastName'):
    ...
    def type_email(self,email_class='seleniumEmailAddress',
                    conf_em_class='seleniumConfirmEmailAddress'):
...
class BookIntl(Book):
    ...
    def set_card(self):
        ...
        if pay_flag:
           ...
           if pay_type == 'random':
              pay_type = random.choice(settings.payment_methods['International']
                                        .keys())
...
class BookDomestic(Book):
    ...
    def type_name(self):
        self.log("Typing driver name")
        first_name = self.engine.find(
                    id='billingForm.travelerForm.driverFirstName')
        first_name.clear()
        fn = self.customer['first_name']
        first_name.send_keys(self.customer['first_name'])
        last_name = self.engine.find(
                    id='billingForm.travelerForm.driverLastName')
        last_name.clear()
        last_name.send_keys(self.customer['last_name'])
    ...
    def type_email(self):
        ...
        email_field.clear()
        email_field.send_keys(email)
        conf_email_field = self.engine.find(
                                id='billingForm.travelerForm._NAE_confirmEmail')
        conf_email_field.clear()
        conf_email_field.send_keys(email)
    ...
    def set_insurance(self,timer=0):
        insurance_header =  self.engine.find(
                            class_name='protectionWrapper').text
    ...
    def check_avilable_payments(self):
        labels = self.engine.find(id="paymentSection").find('label')
        return [l.text for l in labels]

    def set_payment_info(self):
        self.log("Filling payment fields")
        if self.pay_type == 'random':
          self.pay_type = random.choice(
                                settings.payment_methods['Domestic'].keys())
        ...
        if 'Credit card' in payments or "Credit/Debit card" in payments:
            self.engine.find(id="defaultPmIdNewCC").click()
        if self.params['currency'] != 'USD':
          self.engine.find(
          xpath='//select[@id="cardTypeSelector"]/option[text()="'
          +self.pay_type+'"]').click()
        card_num_field = self.engine.find(
        ...
        if card.get('code'):
          ...
          code_field.send_keys('111')
        fn = self.engine.find(
                            id="billingForm.paymentForm.billingFirstName"
                            ).exclude('[disabled]')
        fn.clear()
        fn.send_keys(self.customer['first_name'])
        ln = self.engine.find(
                            id='billingForm.paymentForm.billingLastName'
                            ).exclude('[disabled]')
        ln.clear()
...
class BookCCF(Book):
    ...
    def check_avilable_payments(self):
        labels = self.engine.find(class_name="paymentMethod").find('label')
        return [l.text for l in labels]
    ...
    def pay_by_saved_card(self):
        ...
        if available_flag:
            self.engine.find(id="CREDIT_CARD_0").click()
            code = self.engine.find(id='SAVED_CREDIT_CARD_0_CVV')
            code.clear()
            code.send_keys("111")
        else:
    ...
    def pay_by_saved_bml(self):
        ...
        if "BARBARA's Bill Me Later" in payments:
            self.engine.find(id="BILL_ME_LATER_2").click()
        else:
    ...
    def pay_by_card(self):
        ...
        payments = self.check_avilable_payments()
        if 'Credit card' in payments or "Credit/Debit card" in payments:
            self.engine.find(id="creditCard").click()
        payment = self.engine.find(id="paymentWrapper")
        if self.pay_type == 'random':
          self.pay_type = random.choice(settings.payment_methods['CCF'].keys())
        ...
        year = payment.find(id="cardExpiryDate-button")
        year.click()
        year.send_keys(Keys.PAGE_DOWN)
    ...
    def pay_by_hotdollars(self):
        ...
        self.engine.find(id="hotDollarsChecked").click()
